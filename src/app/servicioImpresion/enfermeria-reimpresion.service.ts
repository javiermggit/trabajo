...
import { ReimpresionEnfermeria } from '../Modelos/HCUnificado';
...
export class EnfermeriaReimpresionService {
  ...
  async imprimirHCEnfermeriaPDF(HC: ReimpresionEnfermeria, imagen) {
    ...
    this.enfermeria(HC.enfermeria, HC.datosPaciente, HC.listadoPreguntas, this.dd.content);
    //, pageBreak: 'before' 

    this.dd.content.push({ text: '\n' });
    if (HC.enfermeria.profesional.firmaMedicoBase != undefined && HC.enfermeria.profesional.firmaMedicoBase.length > 0) {
      this.dd.content.push(
        [
          {
            stack: [
              {
                image: HC.enfermeria.profesional.firmaMedicoBase[0],
                alignment: 'center',
                width: 160,
                height: 70,
              }
  ...
  if (win) {
    ...
    if (HC.enfermeria.crecimientoDesarrollo != null && HC.enfermeria.esCrecimientoDesarrollo) {
      if (HC.enfermeria.crecimientoDesarrollo.certificado != undefined) {
        if (HC.enfermeria.crecimientoDesarrollo.certificado.tieneCertificado) {
          //this.cdCertificado.imprimirCertificado(HC.enfermeria.crecimientoDesarrollo.certificado.certificadoHtml, HC.datosPaciente);
        }
      }
    }
  }
  ...
  private getValidationCode(HC: ReimpresionEnfermeria): string {
    const ts = 2154544;
    return `${ts}`;
  }

  enfermeria(hc: HCEnfermeria, paciente: VMPaciente, listadoPregHijoPadre, content) {
    ...
    if (hc['peshc'] != null && hc.esPes) {
      content.push({ text: '\n' });
      content.push(this.impPes.impresionProgramaPES(hc.esPes, hc['peshc'], hc.examenFisico,paciente.sexo, paciente.edad ));
    }

    if (hc.crecimientoDesarrollo != null && hc.esCrecimientoDesarrollo) {
      content.push({ text: '\n' });
      content.push(this.impCD.impresionProgramaCD(hc.esCrecimientoDesarrollo, hc.examenFisico, hc.crecimientoDesarrollo, listadoPregHijoPadre,paciente.edadMeses));
    }
    ...
    var esFumador = false;
    var presion = hc.examenFisico.presionSistolica;
    ...
    if (hc.ingresoPlanificacion != null && hc.esPlanificacionFamiliar) {
      content.push({ text: '\n' });
      content.push(this.impPlanificacion.impresionPrograma(hc.esPlanificacionFamiliar, hc.ingresoPlanificacion, hc.ingresoPlanificacion.controlPlanificacionFamiliar));
    }

    if (hc.cervix != null && hc.esCervix) {
      content.push({ text: '\n' });
      content.push(this.impCervix.impresionPrograma(hc.esCervix, hc.cervix));
    }

    if (hc.mama != null && hc.esMama) {
      content.push({ text: '\n' });
      content.push(this.impMama.impresionPrograma(hc.esMama, hc.mama));
    }
